...
function hourlyForecast(report, nowMs, hours) {
  ...
  ;["sunrise", "sunset"].forEach(function(kind) {
    ...
    for (var j = 0; j < times.length; j++) {
      ...
      if (!isFinite(time) || time < now || time >= end) continue
      entries.push({
        kind: kind,
        time: time,
  ...
  entries.sort(function(a, b) { return a.time - b.time || (a.kind === "hour" ? -1 : 1) })
  return entries
}

function hourLabel(timestamp, showMinutes) {
  var h = Number(timestamp.slice(11, 13))
  var minutes = timestamp.slice(14, 16)
  return (h % 12 || 12) + (showMinutes ? ":" + minutes : "") + (h < 12 ? " AM" : " PM")
}

function hourlyTemperature(value, imperial) {
  var rounded = roundedTemp(imperial ? celsiusToFahrenheit(value) : value)
  return rounded === "" ? "\u2014" : rounded + "\u00b0"
}

function hourlyPrecipitation(value, imperial) {
  if (value === null || value === undefined || value === "" || !isFinite(Number(value))) return "\u2014"
  return imperial ? (Number(value) / 25.4).toFixed(2) + " in" : Number(value).toFixed(1) + " mm"
}
